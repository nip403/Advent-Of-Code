...
class Solution:
    def __init__(self, *, test_data: Optional[str] = None, memoization_type: Optional[type] = None) -> None:
        with open(Path(inspect.stack()[1].filename).parent / "input_data.txt", "r") as f:
            self.data = self.process_data(f.read())
            
        self.test_data = self.process_data(test_data)
    ...
    def process_data(self, data: str) -> List[Any]:
        return data
    ...
    def main(self, use_test_data: bool = False) -> tuple[Union[int, str]]:
        return f"Part 1: {self.part_1(use_test_data)}\nPart 2: {self.part_2(use_test_data)}"
